import etc from "../assets/blueLogo.png";
const Experience = () => {
  return (
    <div className="max-wid lg:px-24 py-12 text-zinc-600 dark:text-zinc-400">
      <h1 className="font-andika font-bold text-2xl sm:text-3xl text-black dark:text-white">
        Work Experience
      </h1>
      <div className="grid md:grid-cols-2 grid-cols-1 gap-10 py-10">
        <div className="flex gap-4">
          <div className="flex flex-col items-center">
            <img
              className="min-w-[80px] h-[80px] border-2 p-1 dark:border-zinc-500 rounded-lg"
              src={etc}
              alt=""
            />
            <p className="bg-zinc-300 dark:bg-zinc-500 w-[1.5px] h-[100px]"></p>
          </div>
          <div>
            <p>
              Lorem ipsum dolor sit amet consectetur adipisicing elit. Doloribus
              animi, nesciunt ipsam error dolores eligendi officiis
              exercitationem. Dolore sunt explicabo quibusdam nulla est
              voluptatibus quaerat, quisquam aliquid odio asperiores quia
              ratione tenetur temporibus sed sint vero, et fuga mollitia optio!
              Nihil molestiae repellendus quo hic error commodi est placeat
              corporis?
            </p>
          </div>
        </div>
        <div className="flex gap-4">
          <div className="flex flex-col items-center">
            <img
              className="min-w-[80px] h-[80px] border-2 p-1 dark:border-zinc-500 rounded-lg"
              src={etc}
              alt=""
            />
            <p className="bg-zinc-300 dark:bg-zinc-500 w-[1.5px] h-[100px]"></p>
          </div>
          <div>
            <p>
              Lorem ipsum dolor sit amet consectetur adipisicing elit. Doloribus
              animi, nesciunt ipsam error dolores eligendi officiis
              exercitationem. Dolore sunt explicabo quibusdam nulla est
              voluptatibus quaerat, quisquam aliquid odio asperiores quia
              ratione tenetur temporibus sed sint vero, et fuga mollitia optio!
              Nihil molestiae repellendus quo hic error commodi est placeat
              corporis?
            </p>
          </div>
        </div>
        <div className="flex gap-4">
          <div className="flex flex-col items-center">
            <img
              className="min-w-[80px] h-[80px] border-2 p-1 dark:border-zinc-500 rounded-lg"
              src={etc}
              alt=""
            />
            <p className="bg-zinc-300 dark:bg-zinc-500 w-[1.5px] h-[100px]"></p>
          </div>
          <div>
            <p>
              Lorem ipsum dolor sit amet consectetur adipisicing elit. Doloribus
              animi, nesciunt ipsam error dolores eligendi officiis
              exercitationem. Dolore sunt explicabo quibusdam nulla est
              voluptatibus quaerat, quisquam aliquid odio asperiores quia
              ratione tenetur temporibus sed sint vero, et fuga mollitia optio!
              Nihil molestiae repellendus quo hic error commodi est placeat
              corporis?
            </p>
          </div>
        </div>
        <div className="flex gap-4">
          <div className="flex flex-col items-center">
            <img
              className="min-w-[80px] h-[80px] border-2 p-1 dark:border-zinc-500 rounded-lg"
              src={etc}
              alt=""
            />
            <p className="bg-zinc-300 dark:bg-zinc-500 w-[1.5px] h-[100px]"></p>
          </div>
          <div>
            <p>
              Lorem ipsum dolor sit amet consectetur adipisicing elit. Doloribus
              animi, nesciunt ipsam error dolores eligendi officiis
              exercitationem. Dolore sunt explicabo quibusdam nulla est
              voluptatibus quaerat, quisquam aliquid odio asperiores quia
              ratione tenetur temporibus sed sint vero, et fuga mollitia optio!
              Nihil molestiae repellendus quo hic error commodi est placeat
              corporis?
            </p>
          </div>
        </div>
      </div>
    </div>
  );
};

export default Experience;
